      ...
      {showForm && (
        <div className="card p-5">
          <form onSubmit={e => { e.preventDefault(); create.mutate(form); }} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <input className="input-base" placeholder="GRN ID *" required value={form.grnId} onChange={e => setForm(p => ({ ...p, grnId: e.target.value }))} />
            <input className="input-base" placeholder="Invoice ID *" required value={form.invoiceId} onChange={e => setForm(p => ({ ...p, invoiceId: e.target.value }))} />
