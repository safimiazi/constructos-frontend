...
import { apiV3, extendedApiClient } from '@/lib/api';
import { PageHeader } from '@/components/ui/page-header';
import { DataTable } from '@/components/ui/data-table';
import { StatusBadge } from '@/components/ui/status-badge';
import { Select } from '@/components/ui/select';
import { Users, Plus, ChevronDown, ChevronUp } from 'lucide-react';
import { useDepartmentOptions } from '@/hooks/use-select-options';
                    ...
                    <tr key={a.id}>
                      ...
                      <td className="py-1 w-36">
                        <Select
                          ...
                          value={a.stage}
                          onChange={stage => moveStage.mutate({ id: a.id, stage })}
...
export default function RecruitmentPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', vacancies: '1', deadline: '', departmentId: '' });
  const qc = useQueryClient();
  const { options: deptOptions, isLoading: dLoading } = useDepartmentOptions();
      ...
      {showForm && (
        <div className="card p-5">
          <form onSubmit={e => { e.preventDefault(); create.mutate({ ...form, vacancies: Number(form.vacancies) }); }} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Select options={deptOptions} value={form.departmentId} onChange={v => setForm(p => ({ ...p, departmentId: v }))} placeholder="Department (optional)" loading={dLoading} clearable label="Department" />
